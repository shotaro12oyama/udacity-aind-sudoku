...
def naked_twins(values):
    ...
    square_nt_boxes = {}
    for box in t_boxes:
        for r in row_peers[box]:
            if values[r] == values[box]:
                row_nt_boxes.update({box: values[r]})
        for c in column_peers[box]:
            if values[c] == values[box]:
                column_nt_boxes.update({box: values[c]})
